...
export const NomineeRequest = () => {
    const [nomineeRequest, setNomineeRequest] = useState([])
    useEffect(() => {
        ProjectService.getAllNominee().then((res) => {
            setNomineeRequest(res.data)
        })
    }, [nomineeRequest])
    const Approve = (id, status) => {
        const sd = status === "Pending" ? "Approved" : status
        ProjectService.nomineeRequestStatus({ _id: id, status: sd }).then((res) => {
            console.log(res.data)
        })
        console.log(id, sd);
    }
    const Reject = (id, status) => {
        ProjectService.nomineeRequestStatus({ _id: id, status: "Rejected" }).then((res) => {
            console.log(res.data)
        })
    ...
    return (
        <>
            ...
            <div className="container">
                <h3 className="text-center">Add Nominee Request</h3>
                {nomineeRequest.length > 0 ?
                    <table className=" col-md-8 mx-auto mt-5">
                        <tr className="border  border-secondary text-center " style={{ height: "50px" }} >
                            ...
                            <td className="border border-success">Equity</td>
                            <td colSpan="2" className="border border-success" className="text-center">Action</td>

                        </tr>
                        <tbody className="mt-3">{nomineeRequest.map((item) => {
                            return <tr className="border border-secondary"><td className="text-center border border-secondary">{item.accountNo}</td><td className="text-center border border-secondary">{item.name}</td><td className="text-center border border-secondary">{item.status}</td><td className="text-center border border-secondary">{item.equity}</td>
                                <td>
